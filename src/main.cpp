...
int main()
{
  ...
  pid.Init(0.15, 0.001, 4.5, true);
  // pid.Init(0.1, 0.001, 4, true);
  pidt.Init(0.3, 0.001, 0.01, false);
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          pid.UpdateError(cte);
          pidt.UpdateError(speed);
          if (pid.sim_reset == false && pidt.sim_reset == false)
          {
            ...
            throttle_value = + pidt.Kp * pidt.p_error + pidt.Kd * pidt.d_error + pidt.Ki * pidt.i_error;            
            throttle_value = 0.4;
            // DEBUG
